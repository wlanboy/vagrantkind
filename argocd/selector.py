...
def select_files(files: list[Path], label: str, name_fn=None) -> list[Path]:
    ...
    for i, f in enumerate(files, 1):
        print(f"  [{i}] {name_fn(f)}")
    print(f"Select {label} to deploy (e.g. 1  or  1,3  or  all  or  Enter to skip):")
...
def main():
    ...
    selected_apps = select_files(apps, "apps", app_short_name)
    selected_globals = select_files(global_events, "global events")
    ...
    print()
    input("Press Enter to apply, Ctrl+C to abort...")
