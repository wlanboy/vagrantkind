...
echo "  Vorbereiten:  kubectl exec -n dbclient $POD -c sysbench -- /scripts/prepare.sh"
echo "  Ausführen:    kubectl exec -n dbclient $POD -c sysbench -- /scripts/run.sh"
echo "  Aufräumen:    kubectl exec -n dbclient $POD -c sysbench -- /scripts/cleanup.sh"
echo ""
